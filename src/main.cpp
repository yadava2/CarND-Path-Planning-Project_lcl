        ...
        if (event == "telemetry") {
          ...
          //double car_x = j[1]["x"];
            ...
            // Behavior Planning
          	...
          	// State machine to plan the behavior

          	if ( car_ahead ) { // there is a Car ahead at less then 30 m
          	    			  if ( !car_left && lane > 0 ) {
          	                      ...
          	                      } else {// reduce the acceleration of the car
          	                    	      speed_diff -= MAX_ACC;
          	                      	     }
          	                  } else { //if there is no car ahead
          	                         if ( lane != 1 ) { // if the car is not in the center lane.
          	                         if ( ( lane == 0 && !car_right ) || ( lane == 2 && !car_left ) ) {
          	                         lane = 1; // if in any other lane come back to center lane.
          	                         }
          	                         }
          	                         if ( ref_vel < MAX_SPEED ) {// maintaining the speed limit.
          	                         speed_diff += MAX_ACC;
          	                         }
          	                  }
           	// created a list of widely spaced (x,y) way-points, evenly spaced at 30m
          	//now integrating these way-points with a spline and fill it with more points that control
          	...
          	//either we will reference the starting points as where the car is or at the previous paths and points
          	double ref_x = car_x;
          	double ref_y = car_y;
          	...
          	if(prev_size < 2)
          	{
          		//Use the two points that make the path tangent to car
          	...
          	//if the previous path's end point as starting reference
          	else
          	{
